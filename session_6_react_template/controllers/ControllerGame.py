...
class ControllerGame:
    ...
    def load_from_json_file(self):
        if os.path.exists("game.json"):
            with open("game.json", "r") as file:
                game_json = file.read()
                self._game = Game.from_json(game_json)
            print("Game loaded")

    def save_to_json_file(self):
        with open("game.json", "w") as file:
            game_json = self._game.to_json(indent=4)
    ...
    def update(self, delta_time: float):
        ...
        it = CollectionGameObjects(game.game_objects, [EnumGameObjectType.BonusStar])
        for bonus_position_x, bonus_position_y, game_object in it:
